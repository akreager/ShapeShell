...
**Extension support is in progress on `feature/extensions`**: Chrome extensions from a shipped supported list (Bitwarden, Drawing Comfort and Rally for Onshape), a toolbar tray, and popups. Phases 0-4 are done: the tray, action popups, a window and tab model for extensions that Electron does not provide, the install pipeline, and the **Manage Extensions** window (menu: Manage Extensions…). That window downloads supported extensions itself — store extensions track their newest release, verified by signature and a permission ceiling; Drawing Comfort comes from a pinned GitHub commit — and installs anything else from a `.crx` or folder only after a "here be dragons" warning. Updates are checked at launch and never installed automatically; an available one lights a dot on the menu item. Bitwarden works end to end — login, 2FA, unlock and autofill — verified on this machine. Drawing Comfort works in a real drawing, including its content scripts in the `production-drawing-*` iframe. A tray click on an extension with no popup now wakes its sleeping service worker, as Chrome does. Remaining: phase 5 (Flatpak). `SHAPESHELL_DEV_EXTENSIONS` still side-loads from source for development. The plan and all measured results live in [docs/extensions-plan.md](docs/extensions-plan.md); `npm run test-extensions` covers the CRX, zip, installer and downloader code (`TEST_NETWORK=1` adds live store and GitHub downloads), `npm run smoke-extensions` is the tray/popup/API/Manage-window regression test (`SMOKE_NETWORK=1` adds a real one-click install), `npm run smoke-launch` proves `npm start` still puts a window on screen, and `npm run ext-survey` re-measures Electron's API gaps.
...
│   │                        #   manage.html + manage-preload.js (Manage Extensions window)
...
│   │                        #   updates.js (store update check + downloads)
...
│                             #   ext-survey, smoke-extensions, smoke-launch (+ fixtures/)
...
- **`require.main === module` is always false here** — in the main process `require.main` is Electron's own internal module, not the entry file. `main()` is therefore guarded by `SHAPESHELL_TEST_HARNESS`, which only `scripts/smoke-extensions.js` sets. Getting this wrong starts the app with no window and no error at all; `npm run smoke-launch` is the regression test.
...
- **Chromium deletes `_metadata/` from any unpacked extension it loads.** Store CRXs ship one, so an installer that writes it gets a tree that changes on first load — which silently failed the "changed on disk" check on every later launch. The installer skips it.
- **An unpacked extension's id is derived from its path**, and `chrome.storage` is keyed by the id. Installs therefore update in place at a fixed directory; a per-version path wipes the extension's data on every update.
- **Installing or removing an extension does not reach Onshape pages already open.** Content scripts are injected as a page loads, so the change applies only after a reload — Chrome behaves the same. The Manage window says so; don't mistake it for an extension failing under Electron, which is how Rally for Onshape first looked.
- **A file dialog cannot be both a file and a folder chooser.** On Linux and Windows, `properties: ['openFile','openDirectory']` silently yields a **folder-only** chooser: files are unselectable and "Open" returns whichever folder is showing. It fails as a wrong-but-plausible path rather than an error, so the caller blames whatever it does with the result — here, the extension allowlist looked like it was rejecting a valid build. Offer two separate actions.
