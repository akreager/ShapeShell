...
| [Rally for Onshape](https://github.com/RileyDavidson-Evans/Rally-for-Onshape) 1.2.29 (MIT), added 2026-09-26 | Web Store CRX (317KB); the store build is byte-identical to the author's CI-built GitHub release, apart from the store's `update_url` | MV3; `storage` only; one content script on `https://*.onshape.com/*`; a page-world bridge script; **no background worker and no popup**, so its toolbar icon does nothing when clicked, as in Chrome — its settings live inside the Onshape page | Low. Added by editing the list alone; no app changes. Verified 2026-09-26 in a live Part Studio: its theme applies, its page bridge hooks Onshape's `$rootScope`, and its floating toolbar renders. |
...
**Changes reach open pages only on reload** (measured 2026-09-26 with Rally for Onshape, both ways). Content scripts are injected as a page loads, so installing an extension leaves an Onshape page that is already open without it, and removing one leaves its scripts running there, until that page is reloaded. Chrome behaves the same. The Manage window says so: a row installed or updated while it is open carries a note, and a removal says it on the status line, since an unsupported extension's row goes with it. The smoke test checks the note on its unsupported install.
...
- **The supported list is the extension experience; anything else installs behind a warning (2026-09-24).** Supersedes the allowlist-only decision below. The two install menu items are replaced by **Manage Extensions…**, a separate window (as browsers do) listing supported extensions. Each row has a source link (Web Store or GitHub, opened in the default browser), install/update, pin and remove. Footer buttons install an unsupported `.crx` or unpacked folder after a "here be dragons" dialog: Cancel is the default, and confirm stays disabled until an "I understand" box is ticked. Once installed, an unsupported extension behaves like any other (pin, remove), with an "Unsupported" badge.
  ...
  - **The first release lists only Bitwarden and Drawing Comfort.** Extending support should mostly mean editing the list, not the app. *Borne out 2026-09-26: Rally for Onshape was added with one list entry and no app changes.*
...
- ~~**Tray clicks are lost when the worker is asleep**~~ FIXED 2026-09-25. Suspected from Rally for Onshape's dead clicks — wrongly, as it turned out: Rally's manifest declares no background worker at all, so those clicks had nowhere to go, in Chrome as well. The bug was real regardless, and the fixture below proves it. For an extension with no popup, `click()` in `src/extensions/manager.js` sent `chrome.action.onClicked` only to an already-running service worker, and Chrome stops an idle MV3 worker after ~30s, so later clicks logged `no running worker to receive a click` and did nothing. Bitwarden hid it because it has a popup. `click()` now wakes the worker with `startWorkerForScope`, as `emitToExtension()` already did. The in-worker shim holds a click that arrives before any `onClicked` listener for up to 5s, and fetches the active tab through the retrying `invoke()`, since a just-woken worker can call before its IPC handler is attached. An extension with neither a popup nor a worker logs that a click does nothing. `npm run smoke-extensions` covers it: once the action probe's worker has stopped, a click must produce a title from a new worker instance.
